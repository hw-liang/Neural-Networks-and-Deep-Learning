...
def gd_step(cost, params, lrate):
    ...
    ### YOUR CODE HERE
    auto_gradient = ag.grad(cost)
    all_gradient = auto_gradient(params)
    new_params = {}
    for key,val in all_gradient.items():
        new_params[key] = params[key] - lrate * val
    return new_params
    ### END CODE
...
class MetaObjective:
    ...
    def __call__(self, params, return_traj=False):
        ...
        trajectory = [params]
        ### YOUR CODE HERE
        inner_objective = InnerObjective(self.x,self.y)
        for i in range(self.num_steps):
            final_cost = inner_objective(params)
            params = gd_step(inner_objective,params,self.inner_lrate)
...
def train():
    ...
    for i in range(OUTER_STEPS):
        ...
        params = gd_step(val_cost,params,OUTER_LRATE)
        x_val, y_val = data_gen.sample_dataset(NDATA)
        ### END CODE
